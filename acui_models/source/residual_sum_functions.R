...
#          The output of these functions is Residual Sum
# URL    : 
#===============================================================================

# RESIDUAL SUM FOR LOGISTIC MODEL
logistic_sum_error <- function(par){
  ...
  RSS = sum((sim-weight), na.rm=TRUE)
  return(RSS)
}

# RESIDUAL SUM FOR GOMPERTZ MODEL
gompertz_sum_error <- function(par){
  # par     = input parameters of the model
  W0    = par[1]
  m0    = par[2]
  k     = par[3]
...
# RESIDUAL SUM FOR QUADRATIC MODEL
quadratic_sum_error <- function(par){
  # par     = input parameters of the model
  a     = par[1]
  b     = par[2]
  c     = par[3]
...
# RESIDUAL SUM FOR VON BERTALANFFY
