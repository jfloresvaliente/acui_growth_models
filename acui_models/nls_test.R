...
## USAR nls FUNCTION, PROBAR,
## Revisar la convergencia de la optimización
...
outpath <- 'F:/marcelo/acui_models/output/' 
dat <- read.table('F:/marcelo/acui_models/input/centros_acui.csv', header = T, sep = ';')
dat$Days <- dat$Days/365
centros <- levels(dat$Center) # obtener los nombre de los centros

# ## Parametros de inicio (no ajustados)
# Linf=575.666
# k  <-  41.9596142
# t0 <- -0.0044663
# 
# ## Formula del modelo
# VB_Apo <- nls(y~Linf*(1-exp(-k*(x-t0))), start=list(Linf=Linf,k=k, t0=t0))
...
for(i in centros){
  ...
  c.0 <- min(serie1$Weigth) * 0.5
  model.0 <- lm(log(Weigth - c.0) ~ Days, data=serie1)
  ...
  ## Formula del modelo Wmax/(1+exp(a-k*t))
  glm(Weigth ~ Days+X2+X3, family=binomial(link='logit'), data=serie1)
  
  Logistic <- nls(y~Wmax/(1+exp(a-k*x)), start=list(Wmax=Wmax,a=a, k=k))
